window.DEVSHELF_DATA = {
  updatedAt: "2026-09-21T02:22:31.020Z",
  totalCount: 81,
  counts: {
    repos: 46,
    apis: 15,
    aiTools: 16,
    cliTools: 19,
    testingQa: 11,
    freeCloud: 10,
    contributors: 9,
    perks: 10,
  },
  resources: [
    {
      name: "Ollama",
      repo: "https://github.com/ollama/ollama",
      category: "Local AI & Inference",
      description:
        "Get up and running with Llama 3.3, Mistral, Qwen, and other large language models locally with a simple CLI and REST API.",
      language: "Go",
      license: "MIT",
      featured: true,
      goodFirstIssues: "https://github.com/ollama/ollama/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Dify",
      repo: "https://github.com/langgenius/dify",
      category: "AI Agents & Workflows",
      description:
        "An open-source LLM app development platform combining AI workflow, RAG pipeline, agent capabilities, and model management.",
      language: "Python / TypeScript",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/langgenius/dify/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "vLLM",
      repo: "https://github.com/vllm-project/vllm",
      category: "High-Throughput Serving",
      description:
        "A high-throughput and memory-efficient inference and serving engine for LLMs with PagedAttention.",
      language: "Python / C++",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/vllm-project/vllm/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Open-WebUI",
      repo: "https://github.com/open-webui/open-webui",
      category: "Web UI & Chat",
      description:
        "User-friendly AI interface designed for self-hosting with complete Ollama and OpenAI-compatible API support.",
      language: "Python / Svelte",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/open-webui/open-webui/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "YYLO",
      repo: "https://github.com/yylo-dev/yylo",
      category: "AI Agents & Local LLMs",
      description:
        "Command-line orchestrator for AI coding agents: runs Claude Code, Codex CLI, and Gemini CLI in parallel git worktrees with Kanban task state, typed merge flows, and per-agent skills.",
      language: "TypeScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/yylo-dev/yylo/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "LocalAI",
      repo: "https://github.com/mudler/LocalAI",
      category: "Local AI & Inference",
      description:
        "Local, OpenAI-compatible REST API for running LLMs, audio transcription, image generation, and embeddings on commodity CPU/GPU.",
      language: "Go / C++",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/mudler/LocalAI/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "AnythingLLM",
      repo: "https://github.com/Mintplex-Labs/anything-llm",
      category: "Local AI & RAG",
      description:
        "The all-in-one Desktop & Docker AI application with full RAG pipeline, multi-agent workspaces, and complete privacy.",
      language: "JavaScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/Mintplex-Labs/anything-llm/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Flowise",
      repo: "https://github.com/FlowiseAI/Flowise",
      category: "AI Agents & Workflows",
      description:
        "Drag & drop user interface for building customized LLM applications, autonomous agents, and RAG pipelines.",
      language: "TypeScript",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/FlowiseAI/Flowise/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Langflow",
      repo: "https://github.com/langflow-ai/langflow",
      category: "AI Agents & Workflows",
      description:
        "Dynamic visual framework for building, testing, and deploying multi-agent AI and RAG applications.",
      language: "Python / TypeScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/langflow-ai/langflow/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Jan",
      repo: "https://github.com/janhq/jan",
      category: "Local AI & Inference",
      description:
        "Open-source desktop alternative to ChatGPT that runs 100% offline on local hardware with zero data leakage.",
      language: "TypeScript / C++",
      license: "AGPL-3.0",
      featured: false,
      goodFirstIssues: "https://github.com/janhq/jan/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "LibreChat",
      repo: "https://github.com/danny-avila/LibreChat",
      category: "Web UI & Chat",
      description:
        "Enhanced, self-hostable open-source AI chat platform unifying OpenAI, Anthropic, Gemini, and local LLMs.",
      language: "TypeScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/danny-avila/LibreChat/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Fabric",
      repo: "https://github.com/danielmiessler/fabric",
      category: "AI CLI & Productivity",
      description:
        "Open-source prompt and automation framework for augmenting humans using AI patterns and modular CLI tooling.",
      language: "Go",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/danielmiessler/fabric/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "Aider",
      repo: "https://github.com/Aider-AI/aider",
      category: "AI Pair Programming & CLI",
      description:
        "AI pair programming in your terminal: edit code in local git repos with Claude 3.5 Sonnet, GPT-4o, or local LLMs with auto-commits.",
      language: "Python",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/Aider-AI/aider/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
      seeking: "Local model benchmark adapters, terminal UX enhancements, documentation",
      contributorsWanted: true,
    },
    {
      name: "OpenHands",
      repo: "https://github.com/All-Hands-AI/OpenHands",
      category: "Autonomous AI Software Engineers",
      description:
        "Open-source platform for software development agents that can write code, fix bugs, run bash commands, and browse documentation autonomously.",
      language: "Python",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/All-Hands-AI/OpenHands/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
      seeking: "Docker sandbox runtime adapters, evaluation benchmarks, docs & translations",
      contributorsWanted: true,
    },
    {
      name: "Continue",
      repo: "https://github.com/continuedev/continue",
      category: "AI Code Assistant & Extension",
      description:
        "Leading open-source AI code assistant inside VS Code and JetBrains: bring your own models, embeddings, and context providers.",
      language: "TypeScript",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/continuedev/continue/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
      seeking: "Provider integrations, prompt context templates, good first issues",
      contributorsWanted: true,
    },
    {
      name: "llama.cpp",
      repo: "https://github.com/ggerganov/llama.cpp",
      category: "Local AI & Inference",
      description:
        "High-performance LLM inference in pure C/C++ with minimal setup and state-of-the-art quantization across Apple Silicon and GPUs.",
      language: "C / C++",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/ggerganov/llama.cpp/issues",
      type: "ai",
      section: "AI Agents & Local LLMs",
    },
    {
      name: "GitWhisper",
      repo: "https://github.com/RitualDev-Lab/GitWhisper",
      category: "Git & Version Control",
      description:
        "Local-first & privacy-hardened AI Git commit intelligence with Conventional Commits, AST secret redaction, atomic hunk splitting, and 1-click VS Code extension.",
      language: "TypeScript",
      license: "MIT",
      featured: true,
      goodFirstIssues: "https://github.com/RitualDev-Lab/GitWhisper/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
      seeking: "JetBrains IDE Plugin, Community Custom LLM Adapters, Documentation",
      contributorsWanted: true,
    },
    {
      name: "FlashLane",
      repo: "https://github.com/RitualDev-Lab/FlashLane",
      category: "OS & Hardware Utilities",
      description:
        "Universal, high-speed bootable ISO/IMG USB writer with Rufus parity for Windows, macOS, and Linux. Streaming 4MB chunks, MBR/GPT, BIOS/UEFI, and simulation mode.",
      language: "TypeScript / Electron",
      license: "MIT",
      featured: true,
      goodFirstIssues: "https://github.com/RitualDev-Lab/FlashLane/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
      seeking: "macOS diskutil runner, Linux block flashing, E2E ISO tests, Documentation",
      contributorsWanted: true,
    },
    {
      name: "lazygit",
      repo: "https://github.com/jesseduffield/lazygit",
      category: "Git & Version Control",
      description:
        "Simple, fast terminal UI for git commands with interactive rebasing and visual diff navigation.",
      language: "Go",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/jesseduffield/lazygit/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "ripgrep",
      repo: "https://github.com/BurntSushi/ripgrep",
      category: "Terminal & Search",
      description:
        "Ultra-fast line-oriented search tool that recursively searches directories for a regex pattern while respecting gitignore.",
      language: "Rust",
      license: "MIT / UNLICENSE",
      featured: false,
      goodFirstIssues: "https://github.com/BurntSushi/ripgrep/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "eza",
      repo: "https://github.com/eza-community/eza",
      category: "Terminal & Shell",
      description:
        "A modern, maintained replacement for ls with colors, git status integration, icons, and tree views.",
      language: "Rust",
      license: "EUPL-1.2",
      featured: false,
      goodFirstIssues: "https://github.com/eza-community/eza/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "fzf",
      repo: "https://github.com/junegunn/fzf",
      category: "Terminal & Shell",
      description:
        "General-purpose command-line fuzzy finder with blazing speed and interactive shell bindings.",
      language: "Go",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/junegunn/fzf/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "agent-browser",
      repo: "https://github.com/vercel-labs/agent-browser",
      category: "CLI & Developer Productivity",
      description:
        "Open-source browser automation CLI for AI agents, with a fast Rust core and Node.js fallback for navigation, forms, screenshots, and structured page interaction.",
      language: "Rust / TypeScript",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/vercel-labs/agent-browser/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
      seeking: "Cross-platform terminal enhancements, playwright engine bindings, examples",
      contributorsWanted: true,
    },
    {
      name: "bat",
      repo: "https://github.com/sharkdp/bat",
      category: "Terminal & Shell",
      description:
        "Cat clone with syntax highlighting and Git integration for dozens of programming languages.",
      language: "Rust",
      license: "MIT / Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/sharkdp/bat/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "delta",
      repo: "https://github.com/dandavison/delta",
      category: "Git & Version Control",
      description:
        "A syntax-highlighting pager for git, diff, and grep output with side-by-side views and line numbers.",
      language: "Rust",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/dandavison/delta/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "hyperfine",
      repo: "https://github.com/sharkdp/hyperfine",
      category: "Benchmarking & Performance",
      description:
        "Command-line benchmarking tool with statistical analysis, warmup runs, and parametric sweeps.",
      language: "Rust",
      license: "MIT / Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/sharkdp/hyperfine/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "tldr",
      repo: "https://github.com/tldr-pages/tldr",
      category: "Terminal & Productivity",
      description:
        "Collaborative, simplified community man pages focusing on practical, real-world command examples.",
      language: "Markdown",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/tldr-pages/tldr/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "dust",
      repo: "https://github.com/bootandy/dust",
      category: "Disk & Storage",
      description:
        "A more intuitive version of du in Rust providing instant visual disk usage breakdown in terminal.",
      language: "Rust",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/bootandy/dust/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "procs",
      repo: "https://github.com/dalance/procs",
      category: "System & Process Monitoring",
      description:
        "A modern replacement for ps with color highlighting, multi-column search, and process tree view.",
      language: "Rust",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/dalance/procs/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "bottom",
      repo: "https://github.com/ClementTsang/bottom",
      category: "System & Process Monitoring",
      description:
        "Cross-platform graphical process and system monitor with customizable terminal charts and widgets.",
      language: "Rust",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/ClementTsang/bottom/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "curlie",
      repo: "https://github.com/rs/curlie",
      category: "HTTP & Networking",
      description:
        "The power of curl with the ease of use and human-friendly terminal output of HTTPie.",
      language: "Go",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/rs/curlie/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "Starship",
      repo: "https://github.com/starship/starship",
      category: "Terminal & Shell Prompt",
      description:
        "The minimal, blazing-fast, and infinitely customizable cross-shell prompt for any shell (Bash, Zsh, Fish, PowerShell).",
      language: "Rust",
      license: "ISC",
      featured: false,
      goodFirstIssues: "https://github.com/starship/starship/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
      seeking: "New module additions, shell integrations, localized docs, bug fixes",
      contributorsWanted: true,
    },
    {
      name: "zoxide",
      repo: "https://github.com/ajeetdsouza/zoxide",
      category: "Terminal Navigation",
      description:
        "Smarter cd command inspired by z and autojump: learns your navigation habits to jump to directories in fewer keystrokes.",
      language: "Rust",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/ajeetdsouza/zoxide/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "gh-dash",
      repo: "https://github.com/dlvhdr/gh-dash",
      category: "GitHub Workflow & TUI",
      description:
        "Beautiful terminal user interface dashboard extension for GitHub CLI to manage pull requests and issues effortlessly.",
      language: "Go",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/dlvhdr/gh-dash/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "HTTPie",
      repo: "https://github.com/httpie/cli",
      category: "HTTP & Networking",
      description:
        "Modern, user-friendly command-line HTTP client for API debugging with clean JSON formatting, colorized output, and sessions.",
      language: "Python",
      license: "BSD-3-Clause",
      featured: false,
      goodFirstIssues: "https://github.com/httpie/cli/issues",
      type: "cli",
      section: "CLI & Productivity Tools",
    },
    {
      name: "AutoHeal-QA",
      repo: "https://github.com/RitualDev-Lab/autoheal-qa",
      category: "E2E & Playwright",
      description:
        "100% Free & Local-first agentic self-healing E2E test runner for Playwright.  cloud cost with AST auto-patching and verification loop.",
      language: "TypeScript",
      license: "MIT",
      featured: true,
      goodFirstIssues: "https://github.com/RitualDev-Lab/autoheal-qa/issues",
      type: "testing",
      section: "Testing & QA Reliability",
      seeking: "Cypress Adapter, Additional Local Model Adapters, Visual UI Dashboard",
      contributorsWanted: true,
    },
    {
      name: "Playwright",
      repo: "https://github.com/microsoft/playwright",
      category: "E2E Testing",
      description:
        "Fast and reliable end-to-end testing framework for modern web apps across Chromium, Firefox, and WebKit.",
      language: "TypeScript",
      license: "Apache-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/microsoft/playwright/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Vitest",
      repo: "https://github.com/vitest-dev/vitest",
      category: "Unit Testing",
      description:
        "Blazing fast Vite-native unit test framework with ESM, TypeScript, and out-of-the-box coverage.",
      language: "TypeScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/vitest-dev/vitest/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "k6",
      repo: "https://github.com/grafana/k6",
      category: "Performance & Load Testing",
      description:
        "Modern load testing tool, using Go and JavaScript, for testing system performance and reliability.",
      language: "Go",
      license: "AGPL-3.0",
      featured: false,
      goodFirstIssues: "https://github.com/grafana/k6/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Bruno",
      repo: "https://github.com/usebruno/bruno",
      category: "API Testing & Exploration",
      description:
        "Fast, git-friendly, open-source API client that stores collections directly in your repo via plain text files.",
      language: "JavaScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/usebruno/bruno/issues",
      type: "testing",
      section: "Testing & QA Reliability",
      seeking: "Collection runner plugins, translations, UI improvements, good first issues",
      contributorsWanted: true,
    },
    {
      name: "Hoppscotch",
      repo: "https://github.com/hoppscotch/hoppscotch",
      category: "API Testing & Exploration",
      description:
        "Open-source API development ecosystem, lightweight web & desktop app with WebSocket, GraphQL, and SSE support.",
      language: "TypeScript / Vue",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/hoppscotch/hoppscotch/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Mockoon",
      repo: "https://github.com/mockoon/mockoon",
      category: "Mock APIs & Sandboxes",
      description:
        "The easiest and quickest way to design and run mock REST APIs locally with custom routes, rules, and proxies.",
      language: "TypeScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/mockoon/mockoon/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Artillery",
      repo: "https://github.com/artilleryio/artillery",
      category: "Performance & Load Testing",
      description:
        "Cloud-scale load testing and functional testing platform for HTTP, WebSockets, and gRPC.",
      language: "JavaScript",
      license: "MPL-2.0",
      featured: false,
      goodFirstIssues: "https://github.com/artilleryio/artillery/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Locust",
      repo: "https://github.com/locustio/locust",
      category: "Performance & Load Testing",
      description:
        "Scalable user load testing tool written in Python with real-time web monitoring UI.",
      language: "Python",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/locustio/locust/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "MSW (Mock Service Worker)",
      repo: "https://github.com/mswjs/msw",
      category: "API Mocking & Interception",
      description:
        "Seamless REST and GraphQL API mocking library for browser and Node.js using Service Worker API to intercept requests at the network level.",
      language: "TypeScript",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/mswjs/msw/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Vegeta",
      repo: "https://github.com/tsenart/vegeta",
      category: "Performance & Load Testing",
      description:
        "Versatile HTTP load testing tool and library built in Go for drilling HTTP services with constant request rates.",
      language: "Go",
      license: "MIT",
      featured: false,
      goodFirstIssues: "https://github.com/tsenart/vegeta/issues",
      type: "testing",
      section: "Testing & QA Reliability",
    },
    {
      name: "Open-Meteo",
      url: "https://open-meteo.com",
      category: "Weather & Climate",
      description:
        "Free weather forecast API for commercial and non-commercial use with hourly forecasts and historical weather data.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "10,000 req/day",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "GroqCloud Free Tier",
      url: "https://console.groq.com",
      category: "AI & Machine Learning",
      description:
        "Ultra-low latency open-source LLM inference API (Llama 3.3, DeepSeek, Qwen) with a generous free developer tier.",
      auth: "Free API Key",
      cors: "Yes",
      rateLimit: "30 req/min",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "JSONPlaceholder",
      url: "https://jsonplaceholder.typicode.com",
      category: "Development & Testing",
      description:
        "Free fake REST API for prototyping, mock posts, comments, todos, and users with zero configuration.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "Hugging Face Serverless Inference",
      url: "https://huggingface.co/inference-api",
      category: "AI & Machine Learning",
      description:
        "Instant serverless API access to 100,000+ open-source AI models across NLP, vision, and speech with free monthly quota.",
      auth: "Free API Key",
      cors: "Yes",
      rateLimit: "Free monthly tier",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "ipapi.co",
      url: "https://ipapi.co",
      category: "Geocoding & IP",
      description:
        "Real-time IP geolocation and network threat intelligence API returning JSON country, city, timezone, and ASN data.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "1,000 req/day",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "CoinGecko Free API",
      url: "https://www.coingecko.com/api",
      category: "Finance & Crypto",
      description:
        "Live cryptocurrency prices, market cap, historical volume, exchange stats, and ticker data.",
      auth: "Free API Key",
      cors: "Yes",
      rateLimit: "30 req/min",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "Rest Countries",
      url: "https://restcountries.com",
      category: "Geocoding & IP",
      description:
        "Get detailed information about world countries (flags, borders, capitals, currencies, languages) via REST.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "PokeAPI",
      url: "https://pokeapi.co",
      category: "Games & Media",
      description:
        "All the Pokémon data you'll ever need in one place, easily accessible through a modern RESTful API.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "100 req/min",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "Open Library",
      url: "https://openlibrary.org/developers/api",
      category: "Books & Literature",
      description:
        "Millions of book records, author bios, covers, and ISBN lookups from the Internet Archive.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "ReqRes",
      url: "https://reqres.in",
      category: "Development & Testing",
      description:
        "A hosted REST-API ready to respond to your AJAX requests with simulated realistic user and auth data.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "ipify",
      url: "https://www.ipify.org",
      category: "Geocoding & IP",
      description: "A simple public IP address API with guaranteed 99.99% uptime and zero setup.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "DummyJSON",
      url: "https://dummyjson.com",
      category: "Development & Testing",
      description:
        "Free dummy REST API with products, carts, users, posts, comments, auth, and image placeholder endpoints.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "Nominatim OpenStreetMap",
      url: "https://nominatim.openstreetmap.org",
      category: "Geocoding & IP",
      description:
        "Open-source search engine for OpenStreetMap data, providing forward and reverse geocoding.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "1 req/sec",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "ExchangeRate-API",
      url: "https://www.exchangerate-api.com",
      category: "Finance & Crypto",
      description:
        "Reliable currency conversion and forex exchange rates for 160+ currencies with a permanent free tier.",
      auth: "Free API Key",
      cors: "Yes",
      rateLimit: "1,500 req/month",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "Cat Facts",
      url: "https://catfact.ninja",
      category: "Animals & Fun",
      description:
        "Public REST API providing random cat facts, breeds, and trivia with zero authentication required.",
      auth: "No Key",
      cors: "Yes",
      rateLimit: "Unlimited",
      https: true,
      type: "api",
      section: "Free & Public APIs",
    },
    {
      name: "Supabase",
      url: "https://supabase.com",
      category: "Databases & BaaS",
      description:
        "Open source Firebase alternative providing PostgreSQL, Auth, Instant APIs, Realtime subscriptions, and Storage.",
      freeTier: "500MB DB, 50k monthly active users, 1GB storage",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Neon",
      url: "https://neon.tech",
      category: "Serverless Postgres",
      description:
        "Serverless Postgres with instant branching, bottomless storage, and autoscaling compute to zero.",
      freeTier: "0.5 GB storage, instant branching, scale-to-zero",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Cloudflare Workers",
      url: "https://workers.cloudflare.com",
      category: "Serverless Compute & Edge",
      description:
        "Deploy serverless code instantly across Cloudflare's global edge network with ultra-low latency.",
      freeTier: "100,000 requests/day, KV store, D1 SQL database",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Clerk",
      url: "https://clerk.com",
      category: "Authentication & User Management",
      description:
        "Complete user management and authentication suite for React, Next.js, and modern full-stack web applications.",
      freeTier: "10,000 monthly active users, social logins, MFA",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Resend",
      url: "https://resend.com",
      category: "Transactional Email",
      description:
        "Developer-first email platform for sending transactional emails using clean React components.",
      freeTier: "3,000 emails/month, 100 emails/day, 1 custom domain",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "PocketBase",
      url: "https://pocketbase.io",
      category: "Databases & BaaS",
      description:
        "Open source backend in 1 file consisting of embedded SQLite database with real-time subscriptions, built-in auth, and admin dashboard.",
      freeTier: "100% Free & Open Source, single binary self-hostable",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Appwrite",
      url: "https://appwrite.io",
      category: "Backend-as-a-Service",
      description:
        "Self-hosted backend-as-a-service platform providing developers with core APIs for authentication, databases, storage, and serverless functions.",
      freeTier: "Free Cloud Tier: 2 projects, 75k monthly active users, 2GB storage",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Fly.io",
      url: "https://fly.io",
      category: "Serverless Compute & Global VMs",
      description:
        "Run full-stack apps and databases close to your users globally with lightweight Firecracker microVMs.",
      freeTier: "Free allowance: up to 3 shared-cpu-1x VMs, 3GB persistent storage",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Render",
      url: "https://render.com",
      category: "Cloud Hosting & PaaS",
      description:
        "Unified cloud to build and run web applications, APIs, static sites, and background workers with automated Git deploys.",
      freeTier: "Free static sites, free web services (750 hrs/mo), free PostgreSQL",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "Koyeb",
      url: "https://www.koyeb.com",
      category: "Serverless & Containers",
      description:
        "Serverless platform to deploy full-stack apps and Docker containers globally with built-in CDN, SSL, and private networking.",
      freeTier: "Free Eco tier: 1 web service, 512MB RAM, global edge network",
      type: "cloud",
      section: "Free Cloud & Developer Tiers",
    },
    {
      name: "JetBrains Free Open Source License",
      url: "https://www.jetbrains.com/community/opensource",
      category: "Developer Tools & IDEs",
      description:
        "Free All Products Pack subscription (IntelliJ IDEA Ultimate, WebStorm, PyCharm, CLion, GoLand) for open-source maintainers.",
      perkValue: "Free 1-Year All Products Pack ($779/yr value)",
      eligibility: "Active open-source project (3+ months, non-commercial)",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "Sentry for Open Source",
      url: "https://sentry.io/for/open-source/",
      category: "Error Tracking & Observability",
      description:
        "Free Sentry Business plan for qualifying open-source maintainers to monitor application health, errors, and real-time performance.",
      perkValue: "Free Sentry Business Plan ($312+/yr value)",
      eligibility: "Public open-source repository with OSI-approved license",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "GitHub for Startups",
      url: "https://github.com/enterprise/startups",
      category: "Hosting & Dev Collaboration",
      description:
        "Up to 20 seats of GitHub Enterprise free for 12 months, including GitHub Actions minutes, Advanced Security, and Copilot.",
      perkValue: "Up to 20 Free Enterprise Seats ($5,000+ value)",
      eligibility: "Series A or earlier startups",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "AWS Activate Founders",
      url: "https://aws.amazon.com/activate",
      category: "Cloud Infrastructure",
      description:
        "Free AWS promotional cloud credits, technical support, and architectural guidance for early-stage builders and founders.",
      perkValue: "$1,000 - $100,000 AWS Cloud Credits",
      eligibility: "Self-funded or early-stage startups",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "Google Cloud for Startups",
      url: "https://cloud.google.com/startup",
      category: "Cloud Infrastructure & AI",
      description:
        "Access up to $2,000 in GCP credits in year one, covering Compute Engine, Cloud Run, Vertex AI, and BigQuery.",
      perkValue: "$2,000 in GCP Cloud Credits",
      eligibility: "Early-stage bootstrap / funded startups",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "DigitalOcean Open Source Sponsorship",
      url: "https://www.digitalocean.com/open-source",
      category: "Cloud Hosting & Droplets",
      description:
        "Monthly infrastructure credits to host open-source websites, build runners, test nodes, and documentation servers.",
      perkValue: "Recurring Free Droplet / Bandwidth Credits",
      eligibility: "Public open-source projects with active community",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "Postman Open Source Sponsorship",
      url: "https://www.postman.com/open-source",
      category: "API Development",
      description:
        "Free Postman Team subscription for open-source teams maintaining public APIs and workspaces.",
      perkValue: "Free Postman Team Plan",
      eligibility: "Public API open-source projects",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "Datadog for Open Source",
      url: "https://www.datadoghq.com/partner/open-source/",
      category: "Observability & APM",
      description:
        "Free Datadog Pro infrastructure monitoring, log management, and APM for qualifying open-source projects.",
      perkValue: "Free Datadog Pro Account ($180+/host/yr)",
      eligibility: "Non-commercial OSI-approved open source projects",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "BrowserStack Open Source",
      url: "https://www.browserstack.com/open-source",
      category: "Cross-Browser & Device Testing",
      description:
        "Free BrowserStack testing license for open-source maintainers across 3,000+ real desktop and mobile browsers.",
      perkValue: "Free Unlimited Live & Automate Testing ($348+/yr)",
      eligibility: "Public open-source repository with active development",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
    {
      name: "Docker Open Source Community",
      url: "https://www.docker.com/community/open-source/application",
      category: "Container Registries & CI",
      description:
        "Free Docker Hub Team account with unlimited image pulls, public repositories, and automated security vulnerability scans.",
      perkValue: "Free Docker Hub Team Tier ($108+/yr value)",
      eligibility: "Active OSI-compliant open-source projects",
      type: "perks",
      section: "Developer Discounts & Startup Perks",
    },
  ],
};
